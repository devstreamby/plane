...
"""Aggregation of issue time logs into a report for a date range.

Shared by the session-authenticated app endpoints and the API-key authenticated
public endpoints, so both always agree on the numbers they report.
"""
...
def split_ids(value: Optional[str]) -> Optional[List[str]]:
    if not value:
        return None
...
def build_time_log_report(
    ...
    items, and hours logged on them must still be reported for the requested period.
    Filtering the queryset through ``Issue.issue_objects`` (or re-checking
    ``issue.archived_at``) here would silently drop those hours — it is not allowed.
    """
    ...
    # Projects the caller asked for but is not an active member of. They are not
    # part of the report at all, so surface them instead of truncating silently.
    scoped_project_id_strs = {str(pid) for pid in scoped_project_ids}
    unavailable_project_ids = [pid for pid in (project_ids or []) if pid not in scoped_project_id_strs]
    # Projects the caller is a plain member of: only their own logs are included.
    restricted_project_ids = [str(pid) for pid in member_only_project_ids]
    ...
    if not scoped_project_ids:
        return {
            ...
            "restricted_project_ids": restricted_project_ids,
            "unavailable_project_ids": unavailable_project_ids,
            "entries": [],
            "issues": {},
            "users": {},
        }
    ...
    ws_timezone = Workspace.objects.filter(slug=slug).values_list("timezone", flat=True).first() or "UTC"
    ...
    for log in qs:
        pieces = _split_seconds_by_local_date(log.started_at, log.stopped_at, tz, window_start, window_end)
        if not pieces:
            continue
        ...
        issues[issue_key] = {
            ...
            "state_name": log.issue.state.name if log.issue.state_id else None,
            # Archived work items are part of the report by design; the flag lets
            # consumers tell them apart without a second lookup.
            "archived": log.issue.archived_at is not None,
        }
    ...
    users_qs = (
        ...
        .values("id", "display_name", "first_name", "last_name", "email", "avatar_url")
    )

    # `email` is safe to expose here: the visibility filter above already limits
    # `users` to the caller themselves plus members of projects they administer.
    users = {
        str(row["id"]): {
